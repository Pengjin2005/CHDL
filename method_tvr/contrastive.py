...
def masked_logsumexp(logits: torch.Tensor, mask: torch.Tensor, dim: int = -1):
    ...
    *head, L = logits.shape
    ...
    if valid_rows.any():
        ...
        sel_logits = logits_flat[valid_rows].masked_fill(~mask_flat[valid_rows], float('-inf'))
...
def batch_local_token_frame_loss(
    ...
    all_mask = (query_mask.bool().unsqueeze(-1) &
    ...
    if match_labels.dim() == 2:
        ...
        pos_mask = (query_mask.bool().unsqueeze(-1) &
                    (match_labels > 0).bool().unsqueeze(1))
    else:
        # [B, Lq, Lv]
