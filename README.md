tmux# Video Corpus Moment Retrieval with Contrastive Learning

PyTorch implementation for the paper "Video Corpus Moment Retrieval with Contrastive Learning" (**SIGIR 2021**,
long paper): [SIGIR version](https://dl.acm.org/doi/10.1145/3404835.3462874), [ArXiv version](https://arxiv.org/pdf/2105.06247.pdf).
...
For the features of TVR dataset, please download [tvr_feature_release.tar.gz](https://drive.google.com/file/d/1j4mVkXjKCgafW3ReNjZ2Rk6CKx0Fk_n5/view?usp=sharing) (link is copied from
...
directly download Google Drive files using `wget`. Please refer [TVRetrieval#prerequisites](https://github.com/jayleicn/TVRetrieval#prerequisites) for more details about how the features are extracted if you are
interested.
...
$ CUDA_VISIBLE_DEVICES=1 bash method_tvr/scripts/train.sh tvr video_sub_tef resnet_i3d --exp_id 1024-bit-base
# inference
...
# SPLIT_NAME: [val | test]
$ CUDA_VISIBLE_DEVICES=5 bash method_tvr/scripts/inference.sh MODEL_DIR_NAME SPLIT_NAME
CUDA_VISIBLE_DEVICES=0 bash method_tvr/scripts/inference.sh tvr-video_sub_tef-ooo-2025_10_28_14_57_33s val
```

For more details about evaluation and submission, please refer [TVRetrieval#training-and-inference](https://github.com/jayleicn/TVRetrieval#training-and-inference).
