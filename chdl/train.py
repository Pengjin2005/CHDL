...
from chdl.config import BaseOptions
...
from chdl.inference import eval_epoch, start_inference
from chdl.model import CHDL
from chdl.optimization import BertAdam
...
def train_epoch(model, train_loader, optimizer, opt, epoch_i, training=True):
    ...
    for batch_idx, batch in tqdm(
        ...
        model_inputs = prepare_batch_inputs(
        ...
        if torch.isnan(loss) or torch.isinf(loss):
            print(f"NaN/Inf detected in loss at iteration {batch_idx}")
            print(f"Loss value: {loss}")
            for k, v in loss_dict.items():
                print(f"{k}: {v}")
                if torch.isnan(v) or torch.isinf(v):
                    print(f"  ^^ NaN/Inf detected in {k}")
            # Save problematic batch for analysis
            torch.save(model_inputs, f"debug_batch_{batch_idx}.pt")
            print("Problematic batch saved. Exiting...")
            import sys
        ...
        if training:
            ...
            # Debug: Check for NaN gradients before clipping
            has_nan_grad = False
            ...
            if has_nan_grad:
                print("Warning: NaN gradients detected and cleaned!")
    ...
    if training:
        ...
        print(
            "dataloading_time: max {dataloading_time.max} min {dataloading_time.min} avg {dataloading_time.avg}\n"
...
def train(model, train_dataset, train_eval_dataset, val_dataset, opt):
    ...
    eval_tasks_at_training = (
        opt.eval_tasks_at_training
    )  # VR is computed along with VCMR
    save_submission_filename = "latest_{}_{}_predictions_{}.json".format(
        opt.dset_name, opt.eval_split_name, "_".join(eval_tasks_at_training)
    )
    for epoch_i in trange(start_epoch, opt.n_epoch, desc="Epoch"):
        ...
        if opt.eval_path is not None:
            with torch.no_grad():
                train_epoch(
                    model, train_eval_loader, optimizer, opt, epoch_i, training=False
                )
            ...
            for task_type in ["SVMR", "VCMR"]:
                if task_type in metrics:
                    ...
                    for iou_thd in [0.5, 0.7]:
                        opt.writer.add_scalars(
                            "Eval/{}-{}".format(task_type, iou_thd),
                            {
                                k: v
            ...
            if stop_score > prev_best_score:
                ...
                if opt.max_es_cnt != -1 and es_cnt > opt.max_es_cnt:  # early stop
                    ...
                    logger.info(
                        "Early stop at {} with {} {}".format(
                            epoch_i,
                            " ".join([opt.stop_task] + stop_metric_names),
                            prev_best_score,
                        )
                    )
                    break
...
def start_training():
    ...
    opt.eval_log_txt_formatter = (
        "{time_str} [Epoch] {epoch:03d} [Metrics] {eval_metrics_str}\n"
    )
    ...
    model_config = EDict(
        ...
        lw_rec=opt.lw_rec,  # loss weight for Decorrelation Loss
        lw_q=opt.lw_q,  # loss weight for Quantization Loss
        lw_b=opt.lw_b,  # loss weight for Balance Loss
